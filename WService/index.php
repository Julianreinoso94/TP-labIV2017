...
$app = new Slim\App();
 ...
 $app->post('/archivos', function ($request, $response, $args) {
    ...
    $uploadPath = "imagenes" . DIRECTORY_SEPARATOR . $_FILES[ 'file' ][ 'name' ];
...
$app->post('/sucursalesUp[/{objeto}]', function ($request, $response, $args) {
        ...
        if(count($sucursal->fotos) > 0){
            for ($i = 0; $i < count($sucursal->fotos); $i++ ){
                ...
                unlink($rutaVieja);
                $arrayFoto[]= "imagenes/".$rutaNueva;
            } 
            $sucursal->fotos=json_encode($arrayFoto); 
        } 
        //return $response->write(Local::InsertarLocal($sucursal)); 
        $UnaSuc=new sucursal();
     
    
        $UnaSuc->direccion=$sucursal->direccion;
        $UnaSuc->nombre=$sucursal->nombre;
        $UnaSuc->foto1=$arrayFoto[0];
        $UnaSuc->foto2=$arrayFoto[1];
        $UnaSuc->foto3=$arrayFoto[2];
        $resp=sucursal::InsertarSucursal($UnaSuc);
        return json_encode($resp);
